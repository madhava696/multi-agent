...
class ChatWorkflow:
    def __init__(self) -> None:
        self.memory_service = RedisMemoryService(settings.redis_url, settings.redis_ttl_seconds)#holds the conversation history
        self.cache_service = RedisMemoryService(settings.redis_url, settings.redis_ttl_seconds)#store cached(previously generated) responses(answers)
    ...
    #decorator if enabled, LangFuse can trace the complete workflow
    async def run(self, request: ChatRequest) -> ChatResponse:
        conversation_id = request.conversation_id or str(uuid.uuid4())#if frontend has convo_id else create a new convo_id
        ...
        cached_answer = self.cache_service.get_value(cache_key)#check cache for ans
        stored_context = self.memory_service.get_messages(conversation_id)#to get conversation memory
        ...
        conversation_context = [message.model_dump() for message in request.history] or stored_context #get the previous context of msg's
        ...
        ) #create a state to share the workflow execution
        ...
        if state.route == "greeting":
            ...
            logger.info("Handling parallel route.") #but it is concurrent execution not async
            try:
...
"""
         ...
         YES        NO               │
          │         │                │
          ▼         ▼                ▼
       Return    GraphState ← conversation
                   │
                   ▼
            SupervisorAgent
                   │
        ...
        │          │           │
        ▼          ▼           ▼
    greeting    summary      search
        │          │           │
        │          ▼           ▼
        │    SummaryAgent   SearchAgent
        │          │           │
        │          ▼           ▼
        │      LLMService  SearchService
        │                      │
        │                      ▼
        │                 Elasticsearch
        │                      │
